...
function parseCLIArgs() {
  ...
  const libraries = [
    'ruff', 'mypy', 'pydantic-settings',
    'sqlalchemy asyncpg', 'alembic',
    'python-jose passlib[bcrypt]', 'arq',
    'structlog', 'pytest httpx',
  ];

  return { projectType, projectName, pythonVersion, installPipelines, libraries };
}
...
async function main() {
  ...
  if (cliArgs) {
    answers = cliArgs;
    console.log(`⚡ Modo no interactivo: [${cliArgs.projectType}] → ${cliArgs.projectName}`);
  } else {
    const questions = [
      {
        type: 'select',
      ...
      {
        type: (prev, values) => values.projectType === 'backend' ? 'multiselect' : null,
        name: 'libraries',
        message: 'Selecciona las librerías a instalar (Espacio para desmarcar):',
        choices: [
          { title: 'ruff (Linter & Formatter ultra-rápido basado en Rust)', value: 'ruff', selected: true },
          { title: 'mypy (Verificador de tipos estáticos estricto)', value: 'mypy', selected: true },
          { title: 'pydantic-settings (Validación robusta de variables .env)', value: 'pydantic-settings', selected: true },
          { title: 'sqlalchemy 2.0 & asyncpg (Acceso asíncrono a Postgres)', value: 'sqlalchemy asyncpg', selected: true },
          { title: 'alembic (Gestión de migraciones de Base de Datos)', value: 'alembic', selected: true },
          { title: 'python-jose & passlib[bcrypt] (Seguridad y Tokens JWT)', value: 'python-jose passlib[bcrypt]', selected: true },
          { title: 'arq (Procesamiento de tareas pesadas/ML en segundo plano)', value: 'arq', selected: true },
          { title: 'structlog (Logs profesionales estructurados en JSON)', value: 'structlog', selected: true },
          { title: 'pytest & httpx (Suite completa de Testing asíncrono)', value: 'pytest httpx', selected: true },
        ],
      ...
      },
      {
        ...
        message: '¿Integrar automáticamente las pipelines de CI/CD para GitHub Actions?',
        initial: true,
      },
    ];
  ...
  try {
    ...
    if (answers.projectType === 'backend') {
      ...
      await fs.writeFile(path.join(targetDir, '.python-version'), pythonVersion);
      ...
      try {
        execSync('uv add fastapi uvicorn', { cwd: targetDir, stdio: 'inherit' });
      } catch (err) {
        console.error('❌ Error al instalar dependencias con uv. ¿Está uv instalado?');
        process.exit(1);
      }
      ...
      if (answers.libraries && answers.libraries.length > 0) {
        ...
        execSync(`uv add ${libsToInstall}`, { cwd: targetDir, stdio: 'inherit' });
      }
    }
    ...
    if (answers.installPipelines) {
      ...
      try {
        ...
        console.log('⚠️  No se pudo ejecutar la CLI de pipelines. Asegúrate de tenerla publicada.');
      }
    }
